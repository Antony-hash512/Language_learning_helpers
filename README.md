...
## TODO:
 - [x] Добавить возможность озвучки составленных предложений:
   - [x] Через бесплатный облачный сервис.
 ...
 - [x] Добавить использование цветных фрагментов текста в консоли.
...
3.  **Установите Google Gemini CLI:**
    ...
    ```
    В зависимости от вашей конфигурации `npm`, эта команда может потребовать повышения привелегий примощи `sudo` (или `doas`).
    Но можно и без `sudo`/`doas` настроить запуск команды `gemini` используя дотфайлы текущего пользователя:
    ...
    И добавить `~/.npm-global/bin` в переменную окружения `PATH`.
...
6.  **Установите `sqlite3`, `jq` и `mpv` (если они не установлены):**
