...
После каждого ответа и получения обратной связи от ИИ, происходит ожидание получения команды от пользователя (по умолчанию буквы кириллические, чтобы не приходилось менять раскладку, но будут добавлены и аналоги на латинице, на случай использования языка с латинским алфавитом в качестве нативного):
- `с` - перейти к следующему слову
- `д` - запросить другой контекст для текущего слова
- `п` - показать различные варианты перевода данного предложения
- `л` - включение/отключение логирования
- `в` - выйти из скрипта
